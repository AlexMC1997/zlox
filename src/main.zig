...
test "arithmetic" {
    ...
    try chunk.writeConstant(.{ .e_number = 1.0 }, 1);
    try chunk.writeConstant(.{ .e_number = 2.0 }, 1);
    try chunk.writeOpCode(.OP_MULTIPLY, 1);
    try chunk.writeConstant(.{ .e_number = 3.0 }, 1);
    try chunk.writeOpCode(.OP_ADD, 1);
    try chunk.writeOpCode(.OP_RETURN, 2);
    try vm.interpret(&chunk, writer);
    try expectValue(ValueType.e_number, vm.stack.getLast(), 5);
    ...
    try chunk.writeOpCode(.OP_RETURN, 2);
    try vm.interpret(&chunk, writer);
    try expectValue(ValueType.e_number, vm.stack.getLast(), 7);
    ...
    try chunk.writeOpCode(.OP_RETURN, 2);
    try vm.interpret(&chunk, writer);
    try expectValue(ValueType.e_number, vm.stack.getLast(), 0);
    ...
    try chunk.writeConstant(.{ .e_number = 3.0 }, 1);
    try chunk.writeOpCode(.OP_MULTIPLY, 1);
    try chunk.writeConstant(.{ .e_number = 4.0 }, 1);
    try chunk.writeConstant(.{ .e_number = 5.0 }, 1);
    ...
    try chunk.writeOpCode(.OP_RETURN, 2);
    try vm.interpret(&chunk, writer);
    try expectValue(ValueType.e_number, vm.stack.getLast(), 39.0 / 5.0);
}
...
test "type error" {
    ...
    try chunk.writeConstant(.{ .e_number = 1.0 }, 1);
    try chunk.writeConstant(.{ .e_boolean = true }, 1);
    try chunk.writeOpCode(.OP_ADD, 1);
    try chunk.writeOpCode(.OP_RETURN, 2);
...
test "expression" {
    ...
    try expectValue(ValueType.e_number, vm.stack.getLast(), (3.0 - -3.4 * 4.0 / -(51 + 2.0)));
}
...
test "logic" {
    ...
    try expectValue(ValueType.e_boolean, vm.stack.getLast(), !(true and 2 * 2 > 1.5 + 1.5 or false) or 2 * 2 + 2 < 3 + 3 + 3 or (false or 5 == 5));
}
